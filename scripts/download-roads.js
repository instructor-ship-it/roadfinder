...
function processRoads(features) {
  // Group segments by road_id AND region - roads can span multiple regions
  ...
  for (const f of features) {
    ...
    // Key by road_id + region to handle roads spanning multiple regions
    const key = `${roadId}|${region}`;
    
    let road = roadsMap.get(key);

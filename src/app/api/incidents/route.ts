...
const INCIDENTS_URL =
  'https://services2.arcgis.com/cHGEnmsJ165IBJRM/arcgis/rest/services/WebEoc_RoadIncidents/FeatureServer/1/query';
const CLOSURES_URL =
  'https://services2.arcgis.com/cHGEnmsJ165IBJRM/arcgis/rest/services/WebEoc_RoadClosures/FeatureServer/4/query';
...
function getSeverity(
  ...
  if (
    type.includes('road closed') ||
    type.includes('road closure') ||
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'all' || action === 'incidents') {
      ...
      const seenLocations = new Set(incidents.map((i: RoadIncident) => `${i.road}-${i.location}`));
      ...
      for (const closure of closures) {
        ...
        if (!seenLocations.has(key)) {
          ...
          const severity = getSeverity(
            closure.incidentType,
            closure.closureType,
            closure.closureType.includes('closed') ? 'Road Closure' : 'Lane Closure'
          );
          allIncidents.push({
            fid: closure.fid,
            id: closure.id,
            ...
            trafficCondition: closure.closureType.includes('closed')
              ? 'Road Closure'
              : 'Lane Closure',

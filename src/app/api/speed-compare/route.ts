...
function normalizeRoadName(name: string): string {
  return name
    ...
    .trim()
    .replace(
      /\b(street|st|road|rd|avenue|ave|drive|dr|highway|hwy|freeway|fwy|boulevard|blvd|lane|ln|way|circuit|cct|place|pl|court|ct|parade|pde|crescent|cr|terrace|tce|loop|gateway|gwy|parkway|pwy)\b/g,
      ''
    )
    .trim();
}
...
function compareSpeedLimits(): any {
  ...
  for (const [normalizedName, osmRoad] of osmByName) {
    ...
    if (mrwaZones && mrwaZones.length > 0) {
      ...
      const mrwaSpeeds = [...new Set(mrwaZones.map((z: any) => z.speed_limit))].sort(
        (a, b) => a - b
      );
      ...
      const speedsMatch =
        mrwaSpeeds.length === osmSpeeds.length && mrwaSpeeds.every((s, i) => s === osmSpeeds[i]);
      ...
      if (speedsMatch) {
        ...
        const osmOnlySpeeds = osmSpeeds.filter((s: number) => !mrwaSpeeds.includes(s));
        ...
        mrwa:
          mrwaZones.length > 0
            ? {
                ...
                speeds: [...new Set(mrwaZones.map((z: any) => z.speed_limit))].sort(

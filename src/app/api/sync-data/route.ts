...
export async function GET(request: NextRequest) {
  ...
  if (action === 'download' && region) {
    ...
    try {
      ...
      // Exclude local roads
      whereClause += " AND ROAD_NETWORK_TYPE <> 'Local Road'";
      ...
      while (hasMore) {
        const url = `${MRWA_BASE}/18/query?where=${encodeURIComponent(whereClause)}&outFields=ROAD_ID,ROAD_NAME,ROAD_NETWORK_TYPE,START_SLK,END_SLK,CWY&returnGeometry=true&resultOffset=${offset}&resultRecordCount=${batchSize}&f=json`;
        
        const result = await fetchWithTimeout(url, 25000);
        
        if (!result.ok) {
          console.error(`Failed to fetch ${region} at offset ${offset}:`, result.error);
          // If this is the first request and it failed, return error
          if (offset === 0) {
            return NextResponse.json({
          ...
          // Otherwise, stop fetching but continue with what we have
          break;
        }
      ...
      // Fetch speed zones for this region
      const speedWhere = whereClause; // Same filter
      offset = 0;
      hasMore = true;
      
      while (hasMore) {
        const url = `${MRWA_BASE}/8/query?where=${encodeURIComponent(speedWhere)}&outFields=ROAD_ID,ROAD_NAME,START_SLK,END_SLK,SPEED_LIMIT,CWY&returnGeometry=false&resultOffset=${offset}&resultRecordCount=${batchSize}&f=json`;
      ...
      console.log(`${region}: completed with ${roads.length} roads, ${speedZones.length} speed zones`);

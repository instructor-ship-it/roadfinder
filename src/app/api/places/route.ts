...
// Cache for offline amenities data
let offlineAmenitiesData: {
  hospitals: any[];
  fuelStations: any[];
  toilets: any[];
} | null = null;

function loadOfflineAmenitiesData() {
  if (offlineAmenitiesData) return offlineAmenitiesData;
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      offlineAmenitiesData = {
        hospitals: data.hospitals || [],
        fuelStations: data.fuelStations || [],
        toilets: data.toilets || []
      };
      console.log(`Loaded offline amenities: ${offlineAmenitiesData.hospitals.length} hospitals, ${offlineAmenitiesData.fuelStations.length} fuel stations, ${offlineAmenitiesData.toilets.length} toilets`);
    }
  } catch (e) {
    console.error('Error loading offline amenities data:', e);
  }
  
  return offlineAmenitiesData || { hospitals: [], fuelStations: [], toilets: [] };
}
...
function getOfflinePlaces(targetLat: number, targetLon: number): PlacesResult {
  const offlineData = loadOfflineAmenitiesData();
  ...
  if (!isOffline) {
    try {
      ...
      if (hospitalElements.length === 0 && toiletElements.length === 0 && fuelElements.length === 0) {
        return NextResponse.json(getOfflinePlaces(targetLat, targetLon));
      }

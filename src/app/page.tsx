...
export default function Home() {
  ...
  // Offline data source toggles (RC 1.9.1) - when true, use offline data; when false, use online API
  ...
  const generateWorkZoneReport = () => {
    ...
    if (signageCorridor && signageCorridor.length > 0 && result) {
      ...
      const nearbyIntersections = corridorIntersections.filter(road => 
        result && road.name.toLowerCase() !== result.road_name.toLowerCase()
      )
      ...
      if (nearbyIntersections.length > 0) {
        ...
        nearbyIntersections.forEach(road => {
          const slk = road.intersectionSlk ?? parseFloat(road.distance) + (result?.work_zone?.start_slk || 0)
...
<body>
  ...
  <div class="footer">
    <p>Report generated by TC Work Zone Locator vRC 1.9.1</p>
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-xs text-gray-400">vRC 1.9.1 Intersection Colors {offlineReady && <span className="text-green-400">• Offline Ready</span>}</p>
          </div>
            ...
            <div className="mb-3">
              ...
              {showOfflineData && (
                <div className="space-y-3 mt-2 pl-3 border-l-4 border-blue-500/60">
                  ...
                  {/* Offline Data Source Toggles - RC 1.9.1 */}
                  {offlineStats && (
                      ...
                      <div className="space-y-2">
                        ...
                        <label className="flex items-center justify-between p-2 bg-gray-800 rounded cursor-pointer hover:bg-gray-750">
                          <div className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={offlineToggles.amenities}
                              onChange={(e) => updateOfflineToggle('amenities', e.target.checked)}
                              className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-blue-500 focus:ring-blue-500 focus:ring-offset-gray-800"
                            />
                            <span className="text-sm text-gray-300">Amenities (Hospital, Fuel, Toilet)</span>
                          </div>
                          <span className={`text-xs px-2 py-0.5 rounded ${offlineToggles.amenities ? 'bg-amber-600 text-white' : 'bg-green-600/30 text-green-300'}`}>
                            {offlineToggles.amenities ? 'OFFLINE' : 'ONLINE'}
                          </span>
                        </label>
                      </div>
                    </div>
        ...
        {selectedRoad && startSlk && (
          <Button
            onClick={() => {
              const name = prompt('Enter a name for this location (optional):', `${selectedRoad} @ ${startSlk}`)
              if (name !== null) {
                saveLocation(name)
              }
            }}
                ...
                <div className="px-4 pb-4">
                  <SpeedZoneLayout
                    ...
                    intersections={crossRoads
                      ...
                      .map(road => ({
                        name: road.name,
                        slk: road.intersectionSlk ?? (parseFloat(road.distance) + result.work_zone.start_slk),
                  ...
                  <div className="px-4 pb-4">
                    <div className="space-y-2 text-sm">
                      {crossRoads.filter(road => 
                        ...
                        const intColors = ['#a855f7', '#ec4899', '#14b8a6', '#f97316', '#06b6d4', '#84cc16', '#ef4444', '#8b5cf6'];
              ...
              <div className="bg-gray-800 rounded-lg">
                ...
                {showTraffic && (
                  <div className="px-4 pb-4">
                      ...
                      <div className="mt-4 pt-3 border-t border-gray-700">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="bg-gray-900 rounded p-2">
                            ...
                            {heavyPct > 10 && (
                              <p className="text-xs text-amber-400">→ {reducedVph.toLocaleString()} (reduced)</p>
                            )}
                          </div>
                          <div className="bg-gray-900 rounded p-2">
                    ...
                    {vphBothDir > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-700">
                        <h4 className="text-xs font-semibold text-purple-400 mb-2">🚦 Shuttle Flow Max Length</h4>
                        <div className="bg-gray-900 rounded p-3">
                          <div className="flex items-center justify-between">
                            <span className="text-gray-300">Max single lane section:</span>
                            <span className={`font-bold text-lg ${shuttleFlow.risk ? 'text-amber-400' : 'text-green-400'}`}>
                              {shuttleFlow.length}
                            </span>
                          </div>
                          {shuttleFlow.risk && (
                            <p className="text-xs text-amber-400 mt-1">
                              ⚠️ Requires risk assessment approved by Road Infrastructure Manager
                            </p>
                          )}
                          {heavyPct > 10 && (
                            <p className="text-xs text-amber-400 mt-1">

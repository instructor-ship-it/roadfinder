...
export default function OverridesMapPage() {
  ...
  const [geocodingResults, setGeocodingResults] = useState<{ found: number; notFound: number } | null>(null);
      ...
      if (loadedSigns.length > 0 && roadData.length > 0) {
        ...
        if (signsNeedingGeocode.length > 0) {
          ...
          for (let i = 0; i < updatedSigns.length; i++) {
            ...
            const road = roadData.find(r => 
              r.road_id.trim().toUpperCase() === sign.road_id.trim().toUpperCase()
            );
  ...
  const roadGeometry = useMemo(() => {
    ...
    const road = roads.find(r => r.road_id.trim().toUpperCase() === selectedRoad.trim().toUpperCase());
    return road || null;
  }, [roads, selectedRoad]);

  // Convert signs to map markers
  const signMarkers = useMemo(() => {
    return filteredSigns.filter(s => s.lat && s.lon).map(sign => ({
      ...sign,
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p>{geocodingSigns ? 'Looking up coordinates from road geometry...' : 'Loading map data...'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      {geocodingResults && (geocodingResults.found > 0 || geocodingResults.notFound > 0) && (
        <div className={`border-b p-3 ${geocodingResults.found > 0 ? 'bg-green-900/30 border-green-700' : 'bg-yellow-900/30 border-yellow-700'}`}>
          <div className="max-w-7xl mx-auto flex items-center gap-3">
            {geocodingResults.found > 0 && (
              <span className="text-green-300 text-sm">
                ✓ Auto-filled coordinates for {geocodingResults.found} sign{geocodingResults.found !== 1 ? 's' : ''} from road geometry
              </span>
            )}
            {geocodingResults.notFound > 0 && (
              <span className="text-yellow-300 text-sm">
                ⚠ {geocodingResults.notFound} sign{geocodingResults.notFound !== 1 ? 's' : ''} couldn&apos;t be geocoded (road not in offline data)
              </span>
            )}
            <button 
                ...
                <Popup>
                  <div className="text-gray-900 p-2 min-w-[200px]">
                    ...
                    <div className="space-y-1 text-sm">
                      <div><span className="font-medium">SLK:</span> {sign.slk}</div>
                      <div><span className="font-medium">Type:</span> {sign.sign_type}</div>
                      <div><span className="font-medium">Direction:</span> {sign.direction}</div>
                      <div><span className="font-medium">Speed:</span> {sign.front_speed} km/h</div>
                      {sign.back_speed && (
                        <div><span className="font-medium">Back:</span> {sign.back_speed} km/h</div>
                      )}
                      <div>
                      ...
                      {sign.note && (
                        <div className="text-xs text-gray-500 mt-2">{sign.note}</div>
                      )}
                    </div>
                  </div>
                </Popup>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {filteredSigns.map(sign => (
                <div key={sign.id} className="bg-gray-700 rounded p-3 text-sm">
                  ...
                  <div className="text-gray-300">SLK {sign.slk} | {sign.front_speed} km/h</div>
                  <div className="text-xs text-gray-400">{sign.sign_type} | {sign.direction}</div>
                </div>
              ))}
            </div>
          </div>

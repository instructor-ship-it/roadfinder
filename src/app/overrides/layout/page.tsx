  ...
  const roadLayout = useMemo((): RoadLayout | null => {
    ...
    for (const boundary of boundarySigns) {
      const zoneRepeaters = repeaters.filter(
        (r) => r.slk >= boundary.start_slk && r.slk <= boundary.end_slk!
      );

      // Find end sign (next boundary or sign at end_slk)
      const endSign = boundarySigns.find((b) => Math.abs(b.start_slk - boundary.end_slk!) < 0.1);
    ...
    for (const repeater of repeaters) {
      const inZone = zones.some((z) => repeater.slk >= z.startSlk && repeater.slk <= z.endSlk);
      if (!inZone) {
          ...
          {roadLayout && (
            <div className="flex items-center gap-4 text-sm ml-auto">
              ...
              <span className="text-gray-400">
                Range: {roadLayout.minSlk.toFixed(2)} - {roadLayout.maxSlk.toFixed(2)} km
              </span>
            </div>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-4">
          ...
          {[60, 80, 90, 100, 110].map((speed) => (
            <div key={speed} className="flex items-center gap-1">
              <div className="w-6 h-4 rounded" style={{ backgroundColor: getSpeedColor(speed) }} />
      ...
      <div className="max-w-7xl mx-auto p-4">
        {!roadLayout ? (
          <div className="text-center py-12 text-gray-400">
            {selectedRoad
              ? 'No signs found for this road'
              : 'Select a road to view speed zone layout'}
          </div>
        ) : viewMode === 'diagram' ? (
          /* Diagram View */
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-400">
                {roadLayout.roadId} • {roadLayout.zones.length} speed zones
              </p>
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              ...
              <div className="relative h-8 bg-gray-700 rounded">
                {/* Scale markers */}
                {Array.from({
                  length: Math.ceil(roadLayout.maxSlk) - Math.floor(roadLayout.minSlk) + 1,
                }).map((_, i) => {
              ...
              <div className="relative h-24 bg-gray-700 rounded overflow-hidden">
                ...
                {roadLayout.zones.map((zone) => (
                  <div
                    ...
                    style={{
                      left: `${Math.min(Math.max(slkToPercent(zone.startSlk), 2), 98)}%`,
                ...
                {roadLayout.zones.flatMap((zone) =>
                  zone.repeaters.map((repeater) => (
                    <div
                      ...
                      style={{
                        left: `${Math.min(Math.max(slkToPercent(repeater.slk), 2), 98)}%`,
                        transform: 'translateX(-50%)',
                      }}
                    >
                      <div
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span
                        ...
                        style={{ color: getSpeedColor(zone.speed) }}
                      >
                        {zone.speed} km/h
                      </span>
                    ...
                    <div className="text-sm text-gray-300 space-y-1">
                      <div>
                        SLK: {zone.startSlk.toFixed(2)} → {zone.endSlk.toFixed(2)}
                      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {roadLayout.zones.map((zone, idx) => (
                  <div key={idx} className="flex items-center gap-4 p-3 rounded-lg bg-gray-700">
                    <div
            ...
            {roadLayout.unassignedSigns.length > 0 && (
              <div className="bg-gray-800 rounded-lg p-4 border border-orange-700">
                ...
                <p className="text-sm text-gray-400 mb-3">
                  These signs do not fall within any defined zone. Add zone boundaries to include
                  them.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {roadLayout.unassignedSigns.map((sign) => (
                    <div
                      key={sign.id}
                      className="bg-orange-900/20 rounded p-3 flex items-center gap-3"
                    >

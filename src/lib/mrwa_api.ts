...
const LOCAL_ROAD_URL =
  'https://gisservices.mainroads.wa.gov.au/arcgis/rest/services/OpenData/RoadAssets_DataPortal/MapServer/25/query';
const ALL_ROADS_URL =
...
const INTERSECTIONS_URL =
  'https://gisservices.mainroads.wa.gov.au/arcgis/rest/services/OpenData/RoadAssets_DataPortal/MapServer/6/query'; // Layer 6 has accurate intersection names (NODE_DESCR)
...
function getGeometryBounds(features: any[]): {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
 ...
 */
async function getAccurateIntersectionName(
  lat: number,
  lon: number,
  radiusM: number = 100
): Promise<string | null> {
...
export async function findIntersectingRoads(
  ...
  if (isStateRoad) {
        ...
        for (const f of nodeResult.features) {
          ...
          if (gps && attrs.ROAD) {
            ...
            if (!actuallyCrosses) {
              ...
              console.log(
                `[RoadFilter] Filtering parallel road: ${attrs.ROAD_NAME} at SLK ${nodeInfo.slk.toFixed(2)}`
              );
              continue;
            }
          }
    ...
    for (const [nodeNo, info] of nodeMap) {
      ...
      if (
        nodeName.includes('Start') ||
        nodeName.includes('End') ||
        nodeName.includes('Slip') ||
        nodeName.includes('Dual') ||
        nodeName.includes('Link Rd') ||
        nodeName.includes('Great Eastern Hwy')
      ) {
        continue;
      }
      ...
      const cleanName = accurateNodeName
        .replace(' Slip Rd', '')
        .replace(' Link Rd', '')
        .split(' & ')[0]
        .trim();
        ...
        try {
          ...
          if (localResult.features && localResult.features.length > 0) {
            for (const f of localResult.features) {
              ...
              if (
                f.attributes.ROAD === roadId ||
                f.attributes.ROAD?.toLowerCase() === roadId?.toLowerCase()
              ) {
                continue;
              }
    ...
    try {
      ...
      if (layer6Result.features) {
        for (const f of layer6Result.features) {
          ...
          // Skip generic node names
          if (
            nodeName.toLowerCase().includes('end road') ||
          ...
          const roads = nodeName.split(' & ').map((r: string) => r.trim());
    ...
    try {
      ...
      if (intResult.features) {
        for (const f of intResult.features) {
          ...
          const crossParts = parts.filter(
            (p: string) => !p.toLowerCase().includes(baseRoadName.toLowerCase())
          );
...
export async function findNearestIntersections(
  ...
  for (const f of result.features) {
    ...
    const distanceM = haversineDistanceKm(lat, lon, intLat, intLon) * 1000;
            ...
            for (let j = 1; j < refPath.length; j++) {
              ...
              if (segmentsIntersect(rx1, ry1, rx2, ry2, cx1, cy1, cx2, cy2)) {
                ...
                const dist = Math.sqrt(
                  Math.pow(intLat - intersectionLat, 2) + Math.pow(intLon - intersectionLon, 2)
                );

                if (dist <= tolerance * 2) {
                  // Allow 2x tolerance for intersection verification

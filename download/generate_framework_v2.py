...
Version 2 — Expanded with Safety II foundation, cultural reproduction,
monitoring good behaviour, and practical interventions
"""
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run(f'April 2026 \u2022 Version 2 \u2022 Draft for Review')
run.font.size = Pt(11)
...
doc.add_paragraph(
    'The following table maps MRWA Banner Alerts to the framework, showing which elements were '
    'satisfied and which were absent at the time of each incident.'
)

headers = ['Alert', 'Incident', 'Pillar 1\nCrew', 'Pillar 2\nPlan', 'Pillar 3\nEquipment', 'Practice\nAwareness', 'Root Gap']
rows = [
    ['EQ#57872', 'TC slipped on soft batter, 6:15 AM', '\u2713', '\u2713', '\u2713', '\u2717', 'Conditions changed (low light, soft ground). No scanning of ground surface.'],
    ['EQ#57539', 'Worker tripped on faded wheel stop', '\u2713', '\u2713', '\u2713', '\u2717', 'Gradual hazard not identified. No arrival scan for trip hazards.'],
    ['EQ#57762', 'Sun glare changed mid-job', '\u2713', '\u2713', '\u2713', '\u2717', 'Conditions changed (sun position). No reassessment triggered.'],
    ['EQ#56820', 'Concrete saw strikes worker', '\u2713*', '\u2717', '\u2713', '\u2717', 'SWMS incomplete for task. Prestart form reused. Plan and practice both failed.'],
    ['EQ#57801', 'Chain snaps through windscreen', '\u2713', '\u2717', '\u2717', '\u2717', 'Wrong recovery equipment. No one flagged the substitution. All three gaps.'],
    ['EQ#57937', 'Truck rollover on wet surface', '\u2713', '\u2713', '\u2713', '\u2717', 'Conditions changed (wet road). Speed not adjusted. No reassessment.'],
]
add_styled_table(doc, headers, rows)

doc.add_paragraph('')
p = doc.add_paragraph()
run = p.add_run('* Crew was accredited but SWMS did not cover the specific task.')
run.font.size = Pt(9)
run.font.italic = True
run.font.color.rgb = RGBColor(0x77, 0x77, 0x77)

doc.add_paragraph(
    'The pattern is clear: the three pillars were satisfied in five of six incidents. The practice '
    'of dynamic awareness was absent in all six. The incidents were not caused by a lack of planning '
    'or equipment \u2014 they were caused by the gap between what was planned and what was happening, '
    'and the absence of a practice to bridge it.'
)

doc.add_paragraph(
    'In Safety II terms, these incidents represent failures of adaptive capacity, not failures of '
    'compliance. The crews had the preconditions (the 3 Pillars) but lacked the practice (Dynamic '
    'Awareness) that sustains those preconditions when conditions change. This is precisely the gap '
    'that Safety II identifies and that the framework is designed to address.'
)
